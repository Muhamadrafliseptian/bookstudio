                                ...
                                <div class="row">
                                    <div class="col-12">
                                        ...
                                        <p class="mb-0">Tanggal Sewa:
                                            {{ $item->waktu_pesan }} - {{ \Carbon\Carbon::parse($item->tanggal_pesan)->translatedFormat('d F Y') }}
                                        </p>
                                    ...
                                    <div class="col-12">
                                        ...
                                        @if ($item->payment_status === 'PENDING')
                                            ...
                                            <a href="{{ url($item->invoice_url) }}" class="btn btn-sm btn-primary">Bayar
                                                Sekarang</a>
                                        @else
                                            <p class="mb-0">Kode Booking: <b>{{ $item->kode_booking == null ? $item->external_id : $item->kode_booking }}</b></p>
                                            <p class="mb-0">Status Pembayaran: <span
                                                    class="text-success"><b>{{ $item->payment_status }}</b></span></p>
                                            <p class="mb-0">Metode Pembayaran: <b>{{ $item->kode_booking == null ? "KLAIM_REWARD" : $item->payment_method }}</b></p>
                                            <p class="mb-0">Jenis Pembayaran: <b>{{ $item->kode_booking == null ? "REWARD" : $item->payment_channel }}</b></p>
                                        @endif
                                    </div>
                                </div>
                            </div>
